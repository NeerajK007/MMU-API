...
public class FileSyncServiceImpl implements FileSyncService {
	...
	public String syncFiles(String ServerAuthorization) throws IEMRException, IOException {
		...
		// String createAccessCommand= "net use \\\\"+IP+"\\IPC$
		// /u:"+Domain+"\\"+UserName+" "+Password;
		// String createCopyCommand= "robocopy "+localFolderToSync+"
		// \\\\"+IP+"\\"+serverFolder+" /E /R:3 /log+:" + fileSynclogspath +
		// logFileName;
		// String deleteAccessCommand= "net use \\\\"+IP+"\\IPC$ /delete";

		String[] createAccessCommandArr = new String[] { "net use \\\\", IP, "\\IPC$ /u:", Domain, "\\", UserName, " ",
				Password };
		String[] createCopyCommandArr = new String[] { "robocopy ", localFolderToSync, " \\\\", IP, "\\", serverFolder,
				"  /E  /R:3 /log, :", fileSynclogspath, logFileName };
		String[] deleteAccessCommandArr = new String[] { "net use \\\\", IP, "\\IPC$ /delete" };

		logger.info(
				"fileSync createAccessCommand - >" + Arrays.stream(createAccessCommandArr).reduce("", String::concat));
		logger.info("fileSync createCopyCommand - >" + Arrays.stream(createCopyCommandArr).reduce("", String::concat));
		logger.info(
				"fileSync deleteAccessCommand - >" + Arrays.stream(deleteAccessCommandArr).reduce("", String::concat));

		Runtime rt = Runtime.getRuntime();
		FileInputStream fstream = null;

		try {
			Process pr1 = rt.exec(createAccessCommandArr);
			Process pr2 = rt.exec(createCopyCommandArr);
			Process pr3 = rt.exec(deleteAccessCommandArr);
